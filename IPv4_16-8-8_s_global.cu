...
// A structure representing a single node in a search tree (Trie), aligned to 16 bytes for faster loading
...
// Converts an IP address in string format (e.g., "192.168.1.1") to a 32-bit number
...
// CUDA kernel for finding the longest matching pattern (LPM) using global memory on the GPU
...
// Reads prefixes from a text file, converts them to the correct format, and simulates output port mapping
void load_records_from_file(const std::string& filename, std::vector<BGPRecord>& records) {
    ...
    std::cout << "\n[SHIPPING COST ASSIGNMENT STATISTICS (" << NUM_PORTS << " ports)]" << std::endl;
    std::cout << "----------------------------------------------------" << std::endl;
...
// Runs a measurement of the GPU kernel's raw performance (without the influence of the PCIe bus) for various batch sizes
template <typename Alloc>
...
int main() {
    ...
    // === H. SETTING UP STREAMS FOR ASYNCHRONOUS EXECUTION ===
